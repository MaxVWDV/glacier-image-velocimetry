...
beta_deg = [1 13 41 NaN 67 71 81 85 99 110 119 131 145 177 199 220 ...
      291 320 340 355]';
...
alpha_bar = nancirc_mean(alpha_rad);
beta_bar = nancirc_mean(beta_rad);

fprintf('Mean resultant vector:\t%.2f \t%.2f\n', circ_rad2ang([alpha_bar beta_bar]))

alpha_hat = nancirc_median(alpha_rad);
beta_hat = nancirc_median(beta_rad);

fprintf('Median:\t\t\t\t\t%.2f \t%.2f\n', circ_rad2ang([alpha_hat beta_hat]))

R_alpha = nancirc_r(alpha_rad);
R_beta = nancirc_r(beta_rad);

fprintf('R Length:\t\t\t\t\t%.2f \t%.2f\n',[R_alpha R_beta])

S_alpha = nancirc_var(alpha_rad);
S_beta = nancirc_var(beta_rad);

fprintf('Variance:\t\t\t\t%.2f \t%.2f\n',[S_alpha S_beta])

[s_alpha s0_alpha] = nancirc_std(alpha_rad);
[s_beta s0_beta] = nancirc_std(beta_rad);

fprintf('Standard deviation:\t\t%.2f \t%.2f\n',[s_alpha s_beta])
fprintf('Standard deviation 0:\t%.2f \t%.2f\n',[s0_alpha s0_beta])

b_alpha = nancirc_skewness(alpha_rad);
b_beta = nancirc_skewness(beta_rad);

fprintf('Skewness:\t\t\t\t%.2f \t%.2f\n',[b_alpha b_beta])

k_alpha = nancirc_kurtosis(alpha_rad);
k_beta = nancirc_kurtosis(beta_rad);
