...
if ~isempty(fslashes)
  lpath=[ lpath(1:max(fslashes)) './plotting/'];
elseif ~isempty(bslashes)
  lpath=[ lpath(1:max(bslashes)) './plotting\'];
elseif ~isempty(closparantheses)       % for VMS computers only, others don't use ']' in filenames
  lpath=[ lpath(1:max(closparantheses)-1) './plotting]'];
else
  lpath=[ lpath(1:max(colons)) './plotting:'];
end
...
if isempty(w) % Not installed correctly
  disp('**********************************************************');
  disp('* ERROR - Can''t find anything in a /plotting subdirectory *');
