...
if  strcmpi(inputs.stable, 'Yes') 
    ...
    if exist(fullfile(inputs.folder,'stable.png'))  > 0  
        cdata = imread(fullfile(inputs.folder,'stable.png'));
    elseif exist(fullfile(inputs.folder,'stable.jpg')) >0
    ...
    size(parralel_chip,1) == Num_cores | (time_loop == inputs.temporaloversampling && inner_loop == inputs.numimages-time_loop)
        ...
        for inner_loop_parralel = 1:size(parralel_chip,1)
            ...
            if strcmpi(inputs.numpass, 'Multi') 
                [~,~,u,v,snr]=GIVtrackmulti(A1,B1,inputs.realresolution,inputs.windowoverlap);
            elseif strcmpi(inputs.numpass, 'Single') 
                ...
                [u, v, C, Cnoise]=GIVtrack(A1,B1,inputs,max_d,inputs.idealresolution);
            else
            ...
            % implement value
            if strcmpi(inputs.numpass, 'Single') 
                snr = C./Cnoise;
            end
            ...
            [V,~] = xytoV(u, v, mean_resolution, dt);
        ...
        %This section in case one worker in the parralel pool 'dies' during 
        % a long calculation. Should reboot the full parralel pool to ensure
        % maximum speed.
