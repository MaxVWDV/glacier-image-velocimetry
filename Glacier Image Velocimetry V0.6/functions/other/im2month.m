...
mask = ((interp2(images{2,3}, linspace(1, inputs{12,2}(2), inputs{13,2}(2)).', linspace(1, inputs{12,2}(1), inputs{13,2}(1))))); %flipud
                
mask_lin = reshape(mask,[1,inputs{13,2}(1)*inputs{13,2}(2)]);
...
for year_loop = first_year:last_year
    if year_loop == first_year && year_loop ~= last_year
        for month_loop = first_month:12
            ...
            current_month_v = NaN(1,inputs{13,2}(1)*inputs{13,2}(2) + 1);
            
            current_month_fd = NaN(1,inputs{13,2}(1)*inputs{13,2}(2) + 1);
            ...
            weighted_monthly_velocity = zeros(1,inputs{13,2}(1)*inputs{13,2}(2));
            ...
            boundary_crossed(:,1)=[];
                        ...
                        weighted_monthly_fd = zeros(1,inputs{13,2}(1)*inputs{13,2}(2));
            for weighting_loop = 1:size(current_month_fd,1)
                weighted_monthly_fd = weighted_monthly_fd + current_month_fd(weighting_loop,2:end)*current_month_fd(weighting_loop,1);
                        ...
                        weighted_monthly_fd_540 = zeros(1,inputs{13,2}(1)*inputs{13,2}(2));
            ...
            local_weight_fd = zeros(1,inputs{13,2}(1)*inputs{13,2}(2));
            for weighting_loop = 1:size(current_month_fd,1)
                ...
                local_weight_fd = local_weight_fd + portion;
                        ...
                        local_weight_v = zeros(1,inputs{13,2}(1)*inputs{13,2}(2));
            for weighting_loop = 1:size(current_month_v,1)
            ...
            weighted_monthly_velocity = (reshape(weighted_monthly_velocity,inputs{13,2})); %flipud
            
                        weighted_monthly_fd = (reshape(weighted_monthly_fd,inputs{13,2}));%flipud
...
global_standard_deviation_lin = reshape(images_stack{5,2},[1,inputs{13,2}(1)*inputs{13,2}(2)]);

%%%%% Outside iterative loop

for iterations = 1:inputs{50,2}
    ...
    for month_loop  = 1:size(monthly_averages,1)
        ...
        one_month_v_temp = NaN(value_counter,inputs{13,2}(1)*inputs{13,2}(2));
        one_month_fd_temp = NaN(value_counter,inputs{13,2}(1)*inputs{13,2}(2));
...
%     numbering_system_temp = numbering_system_temp+1;
    ...
    elseif (year(datetime(full_time(2,velocity_matrix_loop),'ConvertFrom','datenum'))== year(datetime(full_time(3,velocity_matrix_loop),'ConvertFrom','datenum'))...
        ...
        extra_v_matrix = reshape(monthly_averages{position_in_array_last,3},[1,inputs{13,2}(1)*inputs{13,2}(2)]);
        
        extra_fd_matrix = reshape(monthly_averages{position_in_array_last,4},[1,inputs{13,2}(1)*inputs{13,2}(2)]);
        ...
        for i = 1:inputs{13,2}(1)*inputs{13,2}(2)
        ...
        extra_v_matrix = reshape(monthly_averages{position_in_array_first,3},[1,inputs{13,2}(1)*inputs{13,2}(2)]);
        
        extra_fd_matrix = reshape(monthly_averages{position_in_array_first,4},[1,inputs{13,2}(1)*inputs{13,2}(2)]);
        ...
        %here, revisit)
         for findoutlier = 1:inputs{13,2}(1)*inputs{13,2}(2)
        ...
        one_month_v_temp(numbering_system_temp,:) = reshape(nanfillsm(reshape(nanmean(one_month_v_temp(numbering_system_temp,:),1)...
            ,inputs{13,2}),inputs,2,2),[1,inputs{13,2}(1)*inputs{13,2}(2)]); %fill some of the NaNs
...
%         end
        ...
        previous_iteration_averages_array_v = NaN((position_in_array_last-position_in_array_first)+1,inputs{13,2}(1)*inputs{13,2}(2));
        previous_iteration_averages_array_fd = NaN((position_in_array_last-position_in_array_first)+2,inputs{13,2}(1)*inputs{13,2}(2));
        
        for i = position_in_array_first:position_in_array_last
            if i ~= month_loop %not for current month
                previous_iteration_averages_array_v(i,:) = reshape(monthly_averages{i,3},[1,inputs{13,2}(1)*inputs{13,2}(2)]);
                previous_iteration_averages_array_fd(i+1,:) = reshape(monthly_averages{i,4},[1,inputs{13,2}(1)*inputs{13,2}(2)]);
                
                
            for innerloop = 1:inputs{13,2}(1)*inputs{13,2}(2) %make NaNs same as temp matrix
...
%        numbering_system_temp = numbering_system_temp+1; 
       ...
       monthly_averages_temp{month_loop,3} = (reshape(nanmean(one_month_v_temp,1),inputs{13,2})); %take mean and reshape to right size

       monthly_averages_temp{month_loop,4} = (reshape(nanmean(one_month_fd_temp,1),inputs{13,2})); %take mean and reshape to right size
