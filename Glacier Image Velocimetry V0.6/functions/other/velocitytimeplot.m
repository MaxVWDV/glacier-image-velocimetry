...
x_position = round(inputs{13,2}(2)*(longpoint-inputs{16,2})/(inputs{17,2}-inputs{16,2}));
y_position = round(inputs{13,2}(1)*(latpoint-inputs{14,2})/(inputs{15,2}-inputs{14,2}));

% Check point is not on the edge of the image
if x_position > inputs{13,2}(2) - vtplotinputs{2,2} 
    x_position = inputs{13,2}(2) - vtplotinputs{2,2} ;
end
...
if y_position > inputs{13,2}(1) - vtplotinputs{2,2} 
    y_position = inputs{13,2}(1) - vtplotinputs{2,2} ;
end
...
for position_loop = x_position - vtplotinputs{2,2} : x_position + vtplotinputs{2,2}
    ...
    for position_loop_2 = y_position-y_moved : y_position+y_moved
        all_positions(position_place,1) =  position_loop*inputs{13,2}(1)-position_loop_2+1;
